...
Output is written atomically via a same-directory temporary file: on success `<output>` contains exactly the plaintext;
on any failure an existing file at `<output>` is left unchanged and no partial output remains. On Unix the output file
mode is 0600.
