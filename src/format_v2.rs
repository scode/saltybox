...
//! this binary payload, followed by the literal marker `:end`:
//!
...
//! - ciphertext plus 16-byte Poly1305 tag: everything to end of payload
//!
//! There is no length field: the file is the container, so the sealed data
//! simply runs to the end of the payload.
//!
//! The `:end` marker exists because armored text gets copy-pasted, and a
//! paste that loses its tail is an everyday accident. The marker lets that
//! mistake be reported as exactly what it is — the input is incomplete —
//! instead of surfacing as a puzzling decode or authentication failure.
//! The marker is a transport-layer aid only: it is deliberately NOT
//! covered by any cryptographic check (integrity and authenticity come
//! solely from the AEAD tag), and it carries no information, so there is
//! nothing for an attacker to gain by fabricating it. `:` cannot appear in
//! base64url, so the marker is unambiguous. Trailing whitespace after the
//! marker — any Unicode whitespace, per `str::trim_end` — is ignored: files
//! routinely end with a newline, and a complete unit with a newline after
//! it must not be mistaken for a truncated one.
//!
...
/// Marker terminating saltybox2 armor.
///
/// A plain-text truncation aid for pasted armor, not a cryptographic
/// property; see the module docs for the reasoning.
pub const V2_END_MARKER: &str = ":end";
...
///
/// "Memory cost" is Argon2's main difficulty knob: it sets the size of the
/// work area the KDF must fill and repeatedly traverse, so cost scales with
/// memory used rather than only with CPU iterations. That is what makes
/// brute force expensive even on highly parallel hardware.
...
/// add memory-bandwidth work without any speedup. Enabling that feature
/// was itself rejected: rayon and its thread-pool machinery are a large
/// dependency addition for a tool that deliberately keeps its tree small
/// enough to vendor for emergency recovery, and the payoff would only be
/// shaving part of an already-acceptable interactive pause of roughly a
/// second.
...
impl FormatEngine for V2Engine {
    ...
    fn unarmor(&self, armored: &str) -> Result<Vec<u8>> {
        ...
        // Files and pastes routinely end with a newline; that is not
        // truncation, so trailing whitespace must not trip the marker check.
        let encoded = encoded.trim_end();
        // Checked before base64 so a tail-truncated paste gets this message
        // rather than a decode error. The wording deliberately attributes the
        // finding to the missing plain-text marker: this is a transport aid,
        // and must not read as a cryptographically ensured property.
        let encoded = encoded.strip_suffix(V2_END_MARKER).ok_or_else(|| {
            SaltyboxError::with_kind(
                ErrorCategory::User,
                ErrorKind::ArmoringInvalid,
...
fn derive_key(
    ...
    // Cargo.toml) wipes the main key-equivalent intermediates inside the
    // KDF — the passphrase-derived initial hash H0 and the final block hash
    // that is the direct preimage of the key. The multi-hundred-MiB work
    // buffer itself is freed unwiped by this convenience path, as are some
    // small H0-derived stack temporaries inside the crate. Wiping it would
...
mod tests {
    ...
    /// Build a payload whose header claims the given parameters, over dummy
    /// sealed data. Only useful for exercising validation that fires before
    /// any authentication happens.
    fn payload_claiming_params(m_cost_kib: u32, t_cost: u32, p_cost: u32) -> Vec<u8> {
    ...
    /// configuration cannot hide behind the small test parameters. The
    /// expected values are literals on purpose: they pin the parameters
    /// SPEC.md promises for encrypt, and comparing against the constants
    /// that produced the header would let the spec drift silently.
    #[test]
    ...
    #[test]
    fn test_repeated_encryption_uses_fresh_salt_and_nonce() {
    ...
    /// One violation of every cap and floor, with the exact message each
    /// must produce. Expected messages are asserted exactly so each case
    /// pins its own validation branch: a mis-wired comparison could
    /// otherwise return the right kind from the wrong check. Shared by the
    /// decrypt- and encrypt-path tests so both public entry points are held
    /// to the same contract without assuming they share an implementation.
    fn invalid_param_cases() -> [(u32, u32, u32, &'static str); 6] {
        [
            (
                MAX_M_COST_KIB + 1,
                TEST_T,
    ...
    /// Every cap and floor rejects on the decrypt path via claimed headers,
    /// without running the KDF. (Acceptance at the exact caps and floors is
    /// covered by test_parameter_caps_and_floors_are_inclusive below.)
    #[test]
    fn test_parameter_caps_and_floors_reject() {
        for (m, t, p, expected_msg) in invalid_param_cases() {
            let err = decrypt(b"pw", &payload_claiming_params(m, t, p))
    ...
    /// The caps are inclusive: parameters at exactly the maxima (and the
    /// floors) must validate. Exercised through validate_params directly —
    /// no key derivation runs, so even the 4 GiB memory cap costs nothing
    /// to check here.
    #[test]
    fn test_parameter_caps_and_floors_are_inclusive() {
        validate_params(MAX_M_COST_KIB, MAX_T_COST, MAX_P_COST).unwrap();
        validate_params(8, 1, 1).unwrap();
        validate_params(8 * MAX_P_COST, 1, MAX_P_COST).unwrap();
    }
    ...
    fn test_unarmor_ignores_trailing_whitespace_after_marker() {
        ...
        for suffix in ["\n", "\r\n", " ", "\n\n", "\t\n"] {
            ...
            let payload = V2Engine
                .unarmor(&with_suffix)
                .unwrap_or_else(|e| panic!("unarmor failed for suffix {suffix:?}: {e}"));
            let decrypted = decrypt(b"pw", &payload).unwrap();
            assert_eq!(b"newline tolerant", &decrypted[..], "suffix: {suffix:?}");
        }
    }

    /// The write path must reject every invalid parameter combination the
    /// read path rejects: nothing saltybox writes may be unreadable under
    /// its own validation. Deliberately runs the full case table rather
    /// than one representative case, so the coverage holds even if the two
    /// paths ever stop sharing a validation implementation.
    #[test]
    fn test_encrypt_rejects_invalid_params() {
        for (m, t, p, expected_msg) in invalid_param_cases() {
            let err = encrypt_deterministic(b"pw", b"data", &TEST_SALT, &TEST_NONCE, m, t, p)
                .expect_err(&format!("expected format error on encrypt: {expected_msg}"));
            assert_eq!(err.kind, Some(ErrorKind::BinaryFormat), "{expected_msg}");
            assert_eq!(err.message(), expected_msg);
        }
    }
    ...
    fn test_unarmor_rejects_bad_base64() {
        let err = V2Engine
            .unarmor("saltybox2:bad$$:end")
            .expect_err("expected base64 decode failure");
        assert_eq!(err.kind, Some(ErrorKind::ArmoringDecode));
    }

    /// Any tail loss takes the ":end" marker with it, so armor missing the
    /// marker is diagnosed as truncation with a message naming the missing
    /// plain-text marker — a clear "your input is incomplete" for paste
    /// accidents, never implying a cryptographic check.
    #[test]
    fn test_unarmor_missing_end_marker_is_truncation() {
        let armored = encrypt_deterministic(
            b"pw",
            b"truncation fodder",
            &TEST_SALT,
        ...
        // Cut anywhere in the tail: mid-marker, at the marker boundary, or
        // deep into the base64 body. All lose the marker.
        for cut in [1, 2, ":end".len(), ":end".len() + 10] {
            let truncated = &armored[..armored.len() - cut];
            let err = V2Engine
                .unarmor(truncated)
                .expect_err(&format!("expected marker error cutting {cut} bytes"));
            assert_eq!(err.kind, Some(ErrorKind::ArmoringInvalid), "cut: {cut}");
            assert_eq!(
                err.message(),
                "input does not end with the \":end\" marker; likely truncated",
                "cut: {cut}"
