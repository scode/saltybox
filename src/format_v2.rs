...
fn associated_data(header: &[u8]) -> Vec<u8> {
    let mut aad = Vec::with_capacity(V2_MAGIC.len() + header.len());
    aad.extend_from_slice(V2_MAGIC.as_bytes());
...
pub fn decrypt(passphrase: &[u8], payload: &[u8]) -> Result<Zeroizing<Vec<u8>>> {
    ...
    let m_cost_kib = u32::from_be_bytes(header[SALT_LEN..SALT_LEN + 4].try_into().unwrap());
    let t_cost = u32::from_be_bytes(header[SALT_LEN + 4..SALT_LEN + 8].try_into().unwrap());
    let p_cost = u32::from_be_bytes(header[SALT_LEN + 8..SALT_LEN + 12].try_into().unwrap());
...
mod tests {
    ...
    fn test_parameter_caps_and_floors_reject() {
        let cases = [
            (MAX_M_COST_KIB + 1, TEST_T, TEST_P, "m above cap"),
            (TEST_M, MAX_T_COST + 1, TEST_P, "t above cap"),
            (TEST_M, TEST_T, MAX_P_COST + 1, "p above cap"),
            (TEST_M, 0, TEST_P, "t of zero"),
            (TEST_M, TEST_T, 0, "p of zero"),
            (15, TEST_T, 2, "m below 8 KiB per lane"),
        ];
        for (m, t, p, description) in cases {
            let err = decrypt(b"pw", &crafted_payload(m, t, p))
                .expect_err(&format!("expected format error for {description}"));
            assert_eq!(err.kind, Some(ErrorKind::BinaryFormat), "{description}");
        }
    }
