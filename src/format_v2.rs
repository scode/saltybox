...
fn derive_key(
    ...
    // The work buffer is passphrase-derived throughout — its final block is
    // the direct preimage of the key — so it gets the same wipe-on-drop
    // treatment as the key itself. Managing the buffer here instead of using
    // the crate's hash_password_into convenience path is what makes that
    // possible: the convenience path frees its internal buffer unwiped.
    let mut blocks = Zeroizing::new(vec![Block::default(); block_count]);
    let mut key = Zeroizing::new([0u8; KEY_LEN]);
    argon2
        .hash_password_into_with_memory(passphrase, salt, &mut *key, &mut *blocks)
        .map_err(|e| {
            SaltyboxError::with_kind_and_source(
                ErrorCategory::Internal,
                ErrorKind::Argon2Failure,
