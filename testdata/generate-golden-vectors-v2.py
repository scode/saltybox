...
format specification in SPEC.md.
...
def armor(payload: bytes) -> str:
    return (MAGIC + base64.urlsafe_b64encode(payload).rstrip(b"=")).decode()
...
# Small memory costs keep the Rust test suite fast; the format is identical
# at any parameter values, so nothing is lost by not using write defaults.
VECTORS = [
    vector(
        "empty plaintext",
        b"test",
